...
export default class nuv_vector {
    logger;
    indexes = null;
    constructor(indexes) {
    ...
    async fetchEmbedding(text) {
        try {
            ...
            const response = await axios.post(`https://api.electronhub.top/v1/embeddings`, {
    ...
    calculatePearsonCorrelation(vec1, vec2) {
        ...
        const denominator = Math.sqrt(_.sum(vec1.map((v) => Math.pow(v - mean1, 2))) * _.sum(vec2.map((v) => Math.pow(v - mean2, 2))));
    ...
    async findNearestNeighbors(queryVector, k = 5, method = "cosine", vectorKeyPath, sorting = 'desc', radius, projection, metadataKeyPaths, data, pagination) {
        ...
        for await (const vectorItem of paginatedVectors) {
            const vector = recordPath(vectorItem, vectorKeyPath);
    ...
    checkConvergence(prevCentroids, newCentroids) {
        return prevCentroids.every((prev, idx) => _.isEqual(prev, newCentroids[idx]));
    }
