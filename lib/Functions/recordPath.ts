...
export function recordPath(record: any[], path: string) {
    ...
    for (const key of keys) {
        ...
        if (key.includes('[_') || key.includes('[]')) {
            ...
            if (_.isArray(result)) {
                const arrayField = _.find(result, (item: any) => item.key === arrayKey);
            ...
            if (_.isArray(result)) {
                const field = _.find(result, (item: any) => item.key === key);
                if (field) {
                    if (_.includes(['ObjectArray', 'Object[]', 'Object'], field.type)) {
                        ...
                        result = _.map(field.value, (item: any) => item.value);
                    } else {
                        result = field.value;
                    }
                } else {
